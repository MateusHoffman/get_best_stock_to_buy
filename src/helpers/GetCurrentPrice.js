...
function formatPrices(prices) {
  ...
  const formatDatePrices = prices.map((price) => ({
    ...price,
  ...
  const mostRecentDate = sortPricesByDate[0];
  const threeDaysAgo = moment().subtract(3, 'days').startOf('day');
  const receivedDate = moment.unix(mostRecentDate?.timestamp);
  const isAnyDateGreaterThanThreeDaysAgo = receivedDate.isSameOrAfter(threeDaysAgo);
  if (isAnyDateGreaterThanThreeDaysAgo) {
    return mostRecentDate.price
  } else {
    return null
  }
}

export default async function getCurrentPrice(ticker) {
